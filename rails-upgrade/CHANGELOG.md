...
## Unreleased
- Documented a `workflows/boot-smoke-test-workflow.md` blind spot: the boot smoke test and Step 6's test suite both boot the app but never run `rake assets:precompile`, so a production-only asset-pipeline crash goes undetected until deploy. Found on a live upgrade: `ActiveStorage::Engine`/`ActionCable::Engine`/`ActionText::Engine` unconditionally add their own ES6 JS to `config.assets.precompile` even when unused, crashing an ES5-only minifier (Uglifier) — compounded by a generator-scaffolded-but-unused `app/assets/javascripts/cable.js` dragging the same JS in independently of the official `precompile_assets = false` opt-out flags. Mitigation added: run `assets:precompile` explicitly as part of hop verification when it's part of the app's real deploy path, and delete unused-framework scaffold (`app/channels/application_cable/*`, `cable.js`) rather than leaving it as a latent hazard.
- Added detection pattern for the `cache_format_version = 6.1` deprecation to `detection-scripts/patterns/rails-71-patterns.yml`. Found on a live 7.0 → 7.1 upgrade: the setting is implied by `config.load_defaults 5.1`/`6.0`/`6.1` and is rarely set explicitly, so it warns on every boot under Rails 7.1 with no matching line anywhere in the codebase to grep for — the pattern searches for the explicit form but the fix guidance covers the implicit case too.
