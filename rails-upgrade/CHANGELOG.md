...
- Updated `workflows/direct-detection-workflow.md` to surface the new `kind:` field in detection output (issue #53 rollout, sub-issue #69). Findings are now grouped into two top-level buckets: **fix-before-bump** (`kind: breaking`) and **fix-when-ready** (`kind: deprecation` / `migration` / `optional`). Within each bucket, `priority` (HIGH / MEDIUM / LOW) drives sub-ordering. The kind grouping puts upgrade-blockers at the top of detection output regardless of priority, so a HIGH `deprecation` (silently wrong but not blocking) doesn't compete visually with a HIGH `breaking` (won't boot).
