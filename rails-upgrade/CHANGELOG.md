...
- Added `upgrade-cleanup` companion plugin for finishing a Rails upgrade campaign. Removes dual-boot scaffolding, drops `NextRails.next?` branches, retires stale monkey-patches and version-conditional code, aligns migrations / schema / Dockerfile / CI to the new version baseline, and triages deprecation warnings. Lives as a sibling plugin in this repo (`upgrade-cleanup/.claude-plugin/plugin.json` + `upgrade-cleanup/upgrade-cleanup/SKILL.md` + `upgrade-cleanup/upgrade-cleanup/workflows/upgrade-cleanup-workflow.md`). Delegates `NextRails.next?` removal to the `dual-boot` plugin's cleanup workflow and `load_defaults` alignment to the `rails-load-defaults` plugin. Based on FastRuby.io's "Finishing an Upgrade" methodology. (Closes #1)
