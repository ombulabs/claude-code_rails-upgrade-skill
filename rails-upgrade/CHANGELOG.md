...
## Unreleased
- Added Step 4.7 (Runtime Detection) and `workflows/runtime-detection-workflow.md`. The boot smoke test (Step 4.6) boots lazily, so app-code breakage and app-code deprecations never surface; on apps with strong test coverage that signal is recovered in Step 6, but on apps with thin or no coverage it was lost entirely. The new step eager-loads every app class against `Gemfile.next` to catch load-time breakage under the target (Tier A), runs `zeitwerk:check` on Zeitwerk apps (Tier B), and captures this hop's load-time deprecation warnings by booting the **current** bundle with a behavior lambda that maps each warning to its app `file:line` (Tier C, version-aware across the Rails 7.1+ `deprecators` registry). The bundle split is deliberate: the target tells you what breaks, the current version emits the deprecations to fix before bumping. Documents the hard limit — eager-load runs class bodies, not method bodies, so method-body deprecations still need the test suite / DeprecationTracker — and points at the dual-boot skill's `references/deprecation-tracking.md` for the silencing sweep rather than duplicating it. Wired into `SKILL.md` (resource list + Step 4.7 block) and the boot-smoke-test workflow's handoff.
