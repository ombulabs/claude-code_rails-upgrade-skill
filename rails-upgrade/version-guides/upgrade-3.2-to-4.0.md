...
#### 15. Bidirectional `dependent: :destroy` Now Recurses Forever
...
#### 16. Fixture Dates Must Be Cast to Strings
...
#### 17. `config.eager_load` Required in All Environments
...
#### 18. `config.assets.compress` Removed
...
#### 19. `ActiveSupport::BufferedLogger` Renamed
...
#### 20. `config.paths["config/routes"]` Key Changed
...
#### 21. `select('distinct ...').pluck` → `.distinct.pluck`
...
#### 22. `assign_attributes` Method Signature Changed
...
#### 23. Validation Callback API Changed
...
#### 24. Test Request Headers API Changed
...
#### 25. `ActiveRecord::ImmutableRelation` Error
...
#### 26. PaperTrail Version Models Require `VersionConcern`
...
| `invalid date` in fixtures | Section 16 (Fixture dates) — cast with `.to_s(:db)` |
| `eager_load is set to nil` | Section 17 (config.eager_load) — set in all environments |
| `NameError: uninitialized constant ActiveSupport::BufferedLogger` | Section 19 (BufferedLogger) — renamed to `ActiveSupport::Logger` |
| `ActiveRecord::ImmutableRelation` | Section 25 (ImmutableRelation) — use `.distinct.count` |
| Controller specs don't see custom headers | Section 24 (Test headers) — use `request.headers.merge!` |
