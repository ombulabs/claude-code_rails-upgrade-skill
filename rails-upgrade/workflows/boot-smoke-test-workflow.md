...
- **This step also cannot catch a deploy-target runtime missing a binary the app shells out to** — the boot smoke test, CI, and even a full green test suite all typically run on a *build* image or a developer machine, both of which have a full toolchain. The actual *runtime* dyno/container can be leaner and silently missing something the app depends on at runtime specifically (as opposed to at build/asset-compile time, which the `assets:precompile` bullet above covers). Real example (fastruby/audit): an initializer ran `Bundler::Audit::Database.update!` at every boot to keep a local `git`-cloned advisory database current — this worked for years, until the app's Heroku stack moved to `heroku-26`, whose *runtime* image dropped the `git` binary entirely (only the *build* image still has it). `git clone` failed on every dyno boot, but `Bundler::Audit::Database.update!` itself rescues `DownloadFailed`/`UpdateFailed` and returns `false` — so the app kept booting and serving traffic normally; only the one feature depending on that data (`ArgumentError ("... is not a directory")`) 500'd, and only when actually invoked. Nothing about this was caused by the Rails/Ruby version bump itself — it surfaced *because* finally redeploying a long-dormant app after finishing the upgrade also picks up however much the deploy target's own OS/runtime image had drifted in the meantime, unrelated to app code. Mitigation: (1) grep the app for `system`/backtick/`` ` `` calls and gems known to shell out (`git`, `imagemagick`/`mini_magick`, `wkhtmltopdf`, `ffmpeg`, etc.) before considering a long-overdue upgrade+deploy complete, and confirm each required binary is still present on the actual runtime image (not just the build image) — for Heroku, `heroku run which <binary>` on the target app after deploying, not just a passing build; (2) treat any `rescue` around such a call that doesn't re-raise or log as a standing risk regardless of what triggered this specific investigation — silently swallowed failures there mean the *next* environment drift (stack change, base image change, buildpack change) will be just as invisible next time.
