...
- **This step also cannot catch a production-only `assets:precompile` failure**, because `rails runner` and the test suite both boot the app without ever running the asset pipeline. `ActiveStorage::Engine`, `ActionCable::Engine` and `ActionText::Engine` each unconditionally add their own ES6 JS to `config.assets.precompile` on boot, even when the app uses none of them, and an ES5-only minifier (Uglifier 4.x) fails on it with `Uglifier::Error: Unexpected token`. That only surfaces when `rake assets:precompile` actually runs, which is often at deploy time rather than in CI. A generator-scaffolded but unused `app/assets/javascripts/cable.js` drags the same JS in via `require_tree .` independently of the per-framework `precompile_assets = false` opt-outs. Mitigation: if `assets:precompile` is part of the real deploy path, run it explicitly here (fake `SECRET_KEY_BASE`, `RAILS_ENV=production`, no live DB needed), and delete unused-framework scaffold rather than leaving it as a latent hazard.
