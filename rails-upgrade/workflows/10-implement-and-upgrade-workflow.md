# Workflow 10: Implement and Upgrade
...
- Comprehensive Upgrade Report (from Workflow 08)
- app:update Preview (from Workflow 09)
...
- Test suite passes against both versions. CI config is checked by Workflow 11, which runs next and has its own gate
...
**Do not fix `load_defaults`-triggered runtime deprecation warnings about *future* Rails versions during this hop.** This caveat covers post-bump runtime warnings emitted by Rails X+1 about behavior scheduled to change in X+2 — typically surfaced once `load_defaults X.Y` flips on in Workflow 12. Those belong to the *next* upgrade cycle and are addressed before the next version bump.

This is **not** a contradiction of fix-before-bump. The `kind: deprecation` patterns from Workflow 05's detection are warnings emitted by the *current* Rails version about APIs that go away at the *target* version — they stay in fix-before-bump and should be addressed in this hop.
...
Load `workflows/11-sync-ci-workflow.md`, fix any mismatches before proceeding
